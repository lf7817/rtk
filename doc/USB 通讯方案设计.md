...
- **技术选型**：USB 串口库 [felHR85/UsbSerial](https://github.com/felHR85/UsbSerial)，设备芯片 CH340（CH34x 驱动）。
...
### 2. USB 连接方式
- 使用 [felHR85/UsbSerial](https://github.com/felHR85/UsbSerial) 按串口方式收发。库内置 CH34x 驱动（如 `CH34xSerialDriver`），支持 300～921600 bps，根据 `UsbDevice` 选用 CH34x 驱动即可。
